...
| `source` | the lane and record set the measurement came from, **named rather than pathed**. Earlier versions carried private repository paths here; `docs/disclosure.md` D5 makes structure private, and a path stays useful to a reader long after the number stops being interesting. What is published is the claim, the registered criteria, the numbers, and the verdict |
